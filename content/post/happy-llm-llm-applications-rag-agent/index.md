...
我读这一章时，主要想留下三个模块的清晰印象：评测、RAG 和 Agent。评测帮助判断模型在哪些任务上可靠；RAG 把外部文档接入生成过程；Agent 让模型通过工具参与行动。
...
第七章开头先讲 LLM 评测，我觉得这个安排很合理。因为进入应用层以后，最容易犯的错误就是把模型能力说得太笼统，好像一个模型在某个榜单分数高，就能自然胜任所有场景。
...
文档加载部分支持 `pdf/md/txt`，读出来之后会按 token 长度切成 chunk。这里最值得记的是两个参数：
...
这也是我觉得 RAG 很容易被低估的地方。它表面上像是在调数据预处理，实际上是在决定模型后面能看到什么证据。

### 3.2 向量化层要抽象出来

第七章先定义了 `BaseEmbeddings`，再写 `OpenAIEmbedding`。这个设计挺朴素，但很有教学价值。
...
这也提醒我：RAG 里的模型不止一个。除了最终负责生成回答的 LLM，还有负责把文本映射到向量空间的 embedding 模型。后者质量不好，前者再强也可能拿不到正确上下文。
...
后续真正做项目时，可以把这里替换成 FAISS、Milvus、Qdrant、Chroma 之类的向量库，也可以再补 reranker。但无论外壳怎么换，底层主线仍然是“找到和问题最相关的上下文”。
...
我觉得这部分很像给模型装上一条“证据约束”。RAG 希望模型把答案建立在可追溯材料上。真正做得好不好，还要回到评测里看事实一致性、引用准确性和答案忠实性。

---

## 四、RAG 的学习收获：系统质量不只取决于 LLM

读完 Tiny-RAG 后，我最大的收获是：**RAG 系统的质量，很大一部分取决于生成模型之外的环节。**
...
这一步很重要，因为它改变了 LLM 的角色。它开始承担调度器的职责：
...
第七章也把 Agent 粗略分成任务导向型、规划推理型、多 Agent 系统、探索学习型几类。对我当前学习来说，先抓住任务导向型就够了，因为 Tiny-Agent 主要演示的就是这条最小链路。
...
这一层非常关键。因为模型看到的是结构化 schema。工具描述写得不清楚，参数设计不合理，模型就更容易选错工具或填错参数。
...
这个循环让我更直观地理解了 tool calling。模型会输出一个结构化调用请求，真正执行动作的是外层程序，模型再根据程序返回的结果继续生成。
...
第七章前面介绍了规划、记忆、反思这些能力，但配套 Tiny-Agent 主要还是一个 tool calling demo。它有工具、有消息历史、有二次模型调用，但还没有真正实现复杂规划、长期记忆或反思循环。
...
不过我也顺手记下一个工程边界：Tiny-Agent 中用 `eval(...)` 执行工具调用，适合教学演示，但真实系统里应该换成更安全的白名单分发。模型一旦能触发工具，权限和安全边界就必须认真处理。
...
第七章很适合作为“应用入口”。它用 Tiny-RAG 和 Tiny-Agent 给出了最小可运行骨架，让我先看清系统边界。

---

## 八、写在最后：应用层的核心是把不可靠变得可控

第七章读完后，我对大模型应用的理解更收敛了一点：

> **大模型应用的关键，是在模型外面补上评测、检索、工具、日志和控制边界。**

评测告诉我模型哪里能用，RAG 让回答尽量有依据，Agent 让模型可以接入行动接口。目标很实际：让模型在真实场景里更可控。

所以如果只保留这一章的学习结论，我会记住这三句话：

- 评测负责应用边界。
- RAG 负责外部知识接口。
- Agent 负责工具调度。

学到这里，我对 LLM 应用的关注点也更具体了：系统怎样设计、怎样观测、怎样迭代。
