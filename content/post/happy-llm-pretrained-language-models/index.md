...
这篇文章要解决的问题是：BERT 为什么适合理解任务，T5 为什么适合统一的 text-to-text 任务，GPT/LLaMA 为什么成为大模型时代的主线。复习时先抓住一个判断框架：**模型能看见什么信息、被训练去预测什么、能否高效扩展**。
...
因此比较预训练模型时，不要只记模型名字，要先看它的预训练目标。目标函数会决定模型更偏理解、条件生成，还是开放式续写。
...
BERT 的关键意义，是把 **预训练 + 微调** 推成了 NLP 的主范式。模型先在大规模语料上学习通用表征，再在具体任务数据上微调。
...
如果说 BERT 偏表征学习，GPT 偏自回归生成，那么 T5 的核心方案是：**把所有任务都表述成 text-to-text。**
...
因此可以这样记：**T5 的优势是任务表达统一；Decoder-Only 的优势是生成链路更简洁、更容易扩展。**
...
这条路线验证了一个关键结论：**当 CLM 在足够大的模型和数据上训练时，模型不仅能生成文本，也会在很多理解任务上表现出泛化能力。**
...
所以从历史上看，**LLaMA 的流行来自 GPT 路线进一步工程化后的自然结果。**
...
Tokenizer、Embedding、mask、位置编码、参数共享，这些看似实现细节的设计，都对应明确的建模取舍。复习时要能回答：
...
如果前面对 Transformer 的学习主要回答“模型是怎么工作的”，那么这一篇要回答的是：**不同架构为什么会在不同阶段成为主流。**

BERT 的优势来自双向理解，T5 的优势来自 text-to-text 统一接口，GPT/LLaMA 的优势来自简单一致的生成目标和更直接的规模化路径。GLM 则提醒我们：融合式目标很有启发，但主流路线最终还要接受工程可扩展性的检验。
