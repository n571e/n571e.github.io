...
把视角从 Transformer 的实现细节抬到模型路线之后，我最大的感受是：**预训练语言模型的分化，来自“目标函数 + 信息流方式 + 工程可扩展性”的共同选择。**

真正需要理顺的是几条路线背后的目标函数、信息流方式和扩展逻辑。BERT 为什么会成为 NLU 时代的代表，T5 为什么会以统一任务接口见长，GPT/LLaMA 又为什么会在大模型时代收敛成主线？这篇文章想重新把这条主线讲清楚，同时补上 tokenizer、embedding、causal mask 这些容易被一笔带过、其实非常关键的底层认知。
...
BERT 的意义很大：它把 **预训练 + 微调** 这件事正式推成了 NLP 的主范式。
