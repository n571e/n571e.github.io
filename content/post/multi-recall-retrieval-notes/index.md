...
单独看定义，它们都不难。但真正放进一条检索链路里，就会出现几个问题：
...
这篇是我自己的学习整理，不追求把公式铺满，而是把这些方法在检索系统里的位置讲清楚。
...
这也是我现在理解多路召回的核心：**不是迷信某一种检索方法，而是让不同方法互相补盲区。**

---

## 一、先分清召回和精排

搜索系统一般不会一上来就让最复杂的模型看完整个文档库。文档量一大，这件事成本太高。
...
如果用一个生活化的比喻，召回像先把可能有用的书从书架上拿下来，精排像坐到桌前一页页翻，判断哪本真正能回答问题。
...
所以 BM25 很适合作为基础召回，但它不应该独自承担所有检索任务。

---

## 三、Embedding：把文本放进语义空间

Embedding 的思路和 BM25 不一样。BM25 看词有没有命中；Embedding 会把文本变成一个向量。
...
BM25 如果没有命中关键分词，可能不够稳；Embedding 更可能从整体语义上判断它们相关。
...
所以 Embedding 擅长找“意思像”的东西，但有时不够“字面精确”。这也是它经常要和 BM25 搭配使用的原因。

---

## 四、为什么需要多路召回

BM25 和 Embedding 的优缺点正好互补。
...
我觉得 RRF 的直觉很好：**多路都觉得它不错，那它大概率真的不错。**

---

## 五、Reranker：候选少了以后，再认真判断

如果 BM25、Embedding、SPLADE 是先捞候选，Reranker 就是坐下来认真比较。
...
SPLADE 的吸引力在于，它不像普通 Embedding 那样完全进入黑盒稠密空间，也不像 BM25 那样只依赖原始词面。它可以学习到一些扩展关系。

但它也不是免费的。它需要专门模型，向量维度很高，索引和部署也比 BM25 更复杂。
...
如果业务词很强，比如法规条款、财务科目、商品型号、错误代码，我会加上 BM25：
...
不过我现在不会一开始就把所有东西都加上。检索系统很容易越堆越复杂。更稳的做法是先有一条能跑通的 baseline，再根据错误样本决定补哪一路。

---

## 小结

这次梳理后，我对多路召回的理解清楚了不少。

它不是一个单独算法，而是一种系统设计思路：让不同检索方法从不同角度找候选，再用排序模型做最终筛选。

BM25 解决精确词面匹配，Embedding 解决语义泛化，SPLADE 尝试在稀疏词项空间里加入神经语义扩展，Reranker 负责最后的细粒度判断。

以后再看一条检索链路，我会优先问这几个问题：
...
这些问题比单独背概念更有用。真正的检索效果，往往不是由某个单点算法决定的，而是由整条链路怎么配合决定的。
