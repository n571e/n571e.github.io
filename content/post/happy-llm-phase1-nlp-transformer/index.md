---
title: "从 NLP 基础到 Transformer：我先补的几块地基"
description: "整理文本表示的演化路径，以及手写 Transformer 时最容易卡住的九个问题"
date: 2026-04-12
...
大模型能处理长文本和上下文，前提是**文本表示**已经从稀疏统计特征走到稠密、上下文化向量。复习时我按这条链看：
...
因果掩码用来防止 Decoder 在自回归生成时“偷看未来”。通常这个 `mask` 张量会被构造成一个上三角全为 $-\infty$、下三角（含对角线）全为 $0$ 的矩阵。
把它加到注意力分数 `scores` 后再做 `softmax`，未来位置的权重就会变成 $0$。最后的切片 `:seqlen, :seqlen` 是为了从预分配的大掩码里，取出当前序列长度真正需要的那一块。
